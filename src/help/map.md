...
## ::map-location-dot:: 1 查询谱面信息 !ymmap (!m) {id=map}

可以根据成绩的一些参数（::link:: 连击数、::bullseye:: 准确率）来查询谱面的信息（::magnifying-glass-chart:: 四维、::medal:: PP）。

**使用方法**

!ymmap / m (**`:`游戏模式**) [**谱面编号**] (**准确率`%`**) (**`x`连击数**) (**`-`失误数**) (**`+`模组名称**)

- **::gamepad:: 游戏模式**：谱面的游戏模式，且只能在转谱上赋予模式。请参阅：[游戏模式](./score.md#mode)
- **::hashtag:: 谱面编号**：必填。需要查询的谱面编号 (BID)。
  - 如果不填写，则 Bot 不会提示你（退避）。
- **::bullseye:: 准确率**：预期成绩的准确率。可以输入 0-100 内的值。
  - 如果输入 0-1 之间的小数，则会乘以 100。
  - 如果输入 100-10000 之间的整数，则会除以 100。
  - `%` 作为区分符，可以输入在准确率前后。此外，在大多数输入情况下，可以省略。
  - 不输入默认 100%。
- **::link:: 连击数**：预期成绩的连击数。可以输入大于 1 的整数或 0-1 内的小数。
  - 如果输入 0-1 之间的小数（不包括 0），则表示预期获取谱面最大连击数**多少倍**的连击数。
  - `x` 作为区分符，可以输入在连击数前后。此外，在大多数输入情况下，可以省略。
  - 不输入默认全连击（FC）。
- **::xmark:: 失误数**：预期成绩的失误数（miss）。可以输入大于等于 0 的整数。
  - `-` 作为区分符，可以输入在失误数前后。此外，在大多数输入情况下，可以省略。
  - 不输入默认为 0 失误。
- **::music:: 模组名称**：::music:: 预期成绩内模组的简称，通常为两位大写字母组成。可以无空格地输入多个 ::music:: 模组。
...
## ::headphones:: 2 试听谱面 !ymaudio (!a) {id=audio}
...
- **::hashtag:: 谱面编号**：必填，需要查询的谱面编号 (BID) 或谱面集编号 (SID)。
...
## ::magnifying-glass:: 3 探索谱面 !ymexplore (!e) {id=explore}
...
## ::thumbs-up:: 4 查询过审谱面 !ymqualified (!q) {id=qualified}
...
- **::file-circle-question:: 谱面状态**：谱面目前的状态。
  - 可以输入的参数：graveyard, wip, pending, ranked, approved, qualified, loved。
  - 或者 g, w, p, r, a, q, l。
  - 如果留空，默认是 qualified。
- **::arrow-down-a-z:: 排序**：结果的筛选和排序方式。
  - 可以输入的参数：title, artist, difficulty/star, maprating, plays, ranked/time。
  - 或者 t, a, d/s, m, p, r/t。
  - 以上参数后面不加任何符号或字母，加 `+` 号、加字母 `a`、加 `_asc` 都是 ::arrow-down-a-z:: ==升序==（小的在上）。以上参数后面加 `-` 号、加字母 `d`、加 `_desc` 是 ::arrow-up-a-z:: ==降序==（大的在上）。
    - 常用参数：rd / ranked_desc：按上架时间降序，刚上架的谱面会排在最上面。
  - 如果留空，默认是 ranked_asc：按上架时间升序，此时，刚提名的谱面会排在最上面。

## ::angles-up:: 5 查询上架流程 !ymnomination (!n) {id=nomination}
...
## ::book-bookmark:: 6 查询谱面技巧 !ymmapminus (!mm) {id=mapminus}
...
- **::gamepad:: 游戏模式**：**目前没有效果。** 只支持 Mania 模式的谱面。
- **::hashtag:: 谱面编号**：必填，需要查询的谱面编号 (BID)。
- **::music:: 模组名称**：模组的简称，通常为两位大写字母组成。可以无空格地输入多个 ::music:: 模组。
  - 一般只有倍速模组才会生效。

- **::forward-fast:: 倍速**：如果你想自定义倍速并用来计算，可以在此输入。
  - 支持的倍速范围为：0.1x ~ 5x。
  - 前面的 `x` 可以忽略。
...
## ::list-ol:: 7 查询谱面排行榜 !ymleader (!l) {id=leaderboard}

**使用方法**

!ymleaderboard / l (**l**) (**`:`游戏模式**) (**谱面编号**) (**`#`偏移量或区间**) (**`*`排行榜类型**) (**`+`模组名称**)

- **::l:: 稳定版排行榜**：如果这里填入了一个 **l**，即可查看仅含稳定版客户端成绩的榜单。
- **::gamepad:: 游戏模式**：选填，需要在转谱中指定的 ::gamepad:: 游戏模式。
...
## ::list-ol:: 8 查询群内谱面排行榜 !ymgroupleaderboard (!l) {id=groupleaderboard}
...
## ~~::book-bookmark:: 9 查询课题 !ymcourse (!co)~~ {id=course}
...
## ~~::chart-line:: 10 查询段位单曲 !ymdanacc (!da)~~ {id=danacc}
...
- **::map-pin:: 段位谱面名**：需要查询的段位谱面。
  - 如果记不住段位谱面名，可以留空。Bot 会输出一张列出了所有段位的 ::image:: 图片。
- **::bullseye:: 准确率**：休息段和结束时，记录下来的准确率。可以输入 0-100 内的值，按空格分隔。
  - 如果输入 0-1 之间的小数，则会乘以 100。
...
## ::file-arrow-up:: 11 获取谱面完整背景 !ymgetbackground (!gb) {id=getbg}
...
- **::hashtag:: 谱面编号**：必填，需要获取背景的谱面编号 (BID)。
...
## ::file-arrow-up:: 12 获取谱面背景 !ymgetcover (!gc) {id=getcover}
...
- **::image:: 类别**：需要获取的背景类别。
  ...
  - 如不填写，默认 cover。
- **::hashtag:: 谱面集编号**：必填，需要获取背景的谱面集编号 (SID)。
...
## ::file-arrow-up:: 13 刷新谱面文件 !ymrefreshfile (!rf) {id=refreshfile}
