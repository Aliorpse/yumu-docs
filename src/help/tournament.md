...
## ::elevator:: 1 查询匹配信息 !ymquickplay (!qp) {id=quickplay}
...
- **::slash:: 序号**：需要查询的场次序号（并非房间号）。留空默认查询最近的一场 (#1)。
...
## ::scale-balanced:: 2 查询比赛斗力（评分） !ymrating (!mra/ra) {id=rating}
...
- **::hashtag:: 比赛编号**：必填，需要查询的比赛编号 (MATCHID)。
  - 获取方法：进入房间，点击 #multiplayer 下，BanchoBot 发送的第一条消息中的蓝色 here。之后，在弹出的网站内找到网站链接末尾的数字，即比赛编号 (MATCHID)。
- **::circle-plus:: Easy 倍率**：如果您的比赛对于 Easy 模组有分数加成，请填写此字段（开头加 e）。
  - 允许填写 0~20 内的数字（允许小数）。比如：**e** 1.8。
  - 如不填写，默认为 1（不修改）。
- **::forward:: 跳过开头**：需要跳过的开头对局数量。这适用于比赛含有**热手**等不需要统计玩家表现的开头对局。
  - 如不填写，默认为 0。想要输入忽略结尾的参数时，也可输入 0 来占位。
- **::rotate-left:: 忽略结尾**：需要忽略的结尾对局数量。这适用于比赛含有**决胜局表演或娱乐局**等不需要统计玩家表现的结尾对局。
  - 如不填写，默认为 0。
- **::trash-can:: 筛除对局**：如果您并不能仅依靠以上两个操作就能实现筛除无用对局的目标，也可以输入要筛除的对局所对应的位置。
  - 格式为中括号 `[]` 括起来的数组。从 1 开始。
    - 比如 !ymra matchID [1, 2, 3] = 筛去前三场 = !ymra matchID **3**。
  - 可与以上两个功能搭配使用。
    - 比如 !ymra matchID [1, 2, 3] = !ymra matchID **2** [3]。
- **::person-falling:: f**：如果填入 f，则表示统计玩家低于 1 万分的成绩。
  - 如不填写，默认忽略玩家低于 1 万分的成绩（**默认去除房间内的裁判**）。
- **::slash:: r**：如果填入 r，则表示忽略重复的对局，只保留靠后的那一场。
  - 如不填写，默认统计重复的对局。
...
- **左上角的比赛信息卡片**：
  - 卡片左上角是比赛的 ::heading:: 房间标题。
  - 卡片左下角依次是 ::hashtag:: 对局数量、::clock:: 比赛时间区间以及 ::calendar:: 比赛日期。
  - 卡片右下角依次是比赛 ::star:: 平均星数、::hashtag:: 比赛编号以及 ::clipboard:: 计分板。
- **玩家个人信息卡片**：
  - 卡片左侧是玩家的 ::palette:: 队伍颜色和 ::circle-user:: 头像。
  - 卡片中间靠左依次是玩家的 ::heading:: 昵称、::person-rays:: 比赛总分（总输出）、::chess-king:: 胜负和胜率、::user-plus:: 玩家评分排名和 ::wave-square:: RWS。
    - RWS 指 ::wave-square:: 平均胜利分配（(average) Rounds Win Share），即为了所属队伍的**胜利**做出了多少贡献。有时候玩家评分很高，但不一定很有用。
    - 想要了解更多，请参考 ::link:: [这篇文章](https://baijiahao.baidu.com/s?id=1724882889457381886&wfr=spider&for=pc)。
  - 卡片中间靠右是 ::tags:: 玩家类型和 ::sun:: 玩家评分。
    - 玩家类型：见下。
...
## ::scale-balanced:: 3 查询系列比赛斗力（评分） !ymseries (!sra/sa) {id=series}
...
- **::address-card:: 比赛名称**：选填，这个名字将展现在左上角的卡片中。不填默认显示第一场比赛的名称。
- **::hashtag:: 比赛编号**：同上，必填，需要查询的比赛编号 (MATCHID)。
- **::circle-plus:: Easy 倍率**：同上，选填，如果您的比赛对于 Easy 模组有分数加成，请填写此字段（开头加 e）。
- **::forward:: 跳过开头**：同上，选填，需要跳过的开头对局数量。
  - 如不填写，默认为 0。想要输入忽略结尾的参数时，也可输入 0 来占位。
- **::rotate-left:: 忽略结尾**：同上，选填，需要忽略的结尾对局数量。
  - 如不填写，默认为 0。
- **::trash-can:: 筛除对局**：同上，选填，如果您并不能仅依靠以上两个操作就能实现筛除无用对局的目标，也可以输入要筛除的对局所对应的位置。
- **::person-falling:: f**：如果填入 f，则表示统计玩家低于 1 万分的成绩。
...
需要按组输入 ::hashtag:: 比赛编号、::forward:: 跳过开头和 ::rotate-left:: 忽略结尾、::trash-can:: 筛除对局参数。当然，也可以不输入（默认为 0）。
...
## ::headphones:: 4 监听比赛房间 !ymmatchlisten (!ml) {id=listen}

监听比赛房间，并在比赛开始时给出提示，对局结束时输出 [::chalkboard-user:: 比赛对局信息（MR 功能）](#round)。
...
## ::house-chimney-user:: 5 获取比赛结果 !ymmatchnow (!mn) {id=matchnow}
...
:::
## ::square-xmark:: 6 获取比赛最近成绩 !ymmatchrecent (!mr)  {id=matchrecent}
...
- **::map-marker:: 位置**：如果输入一个值，则会返回这一个位置的成绩。
  - 不输入默认返回 50 个成绩。

## ::chalkboard-user:: 7 查询对局信息 !ymround (!ro) {id=matchround}
...
- **::hashtag:: 比赛编号**：同上。
- **::location-dot:: 对局位置**：需要查询的对局场次位置。
  - 可以输入一个整数。输入 2 就是从开始算起第二局。
- **::key:: 关键字**：如果不知道具体的对局位置，可以忽略，并填写关键字的字段。
...
## ::tent-arrows-down:: 8 生成图池图片 !ymgetpool (!getpool) {id=getpool}
...
- **::address-card:: 比赛名称**：选填，这个名字将展现在左上角的卡片中。
- **::music:: 模组**：同上，必填，模组池。
  - 必须为两个大写字母。
  - 如果模组池的模组会改变 ::star:: 星数（比如 DT），则谱面的 ::star:: 星数等属性会自动随之更改。
  - 必须输入在谱面编号之前。且之后必须至少跟随 1 个谱面编号。
- **::hashtag:: 谱面编号**：同上，必填，输入的谱面 (BID)。
...
需要按组输入 ::music:: 模组、::hashtag:: 谱面编号。并且，至少需要输入 ==1== 个模组，且==之后==必须至少跟随 ==1== 个谱面编号。
...
## ::tent:: ~~9 读取图池 !ymmappool (!po) {id=pool}~~
...
- **::hashtag:: 图池编号**：选填，图池的编号。
- **::address-card:: 图池名称**：选填，图池的名字。如果无法匹配则会返回最接近的结果（？）。
